import type { inquiry } from "@/lib/db/schema"

/**
 * Drizzle-inferred row types for the `inquiry` table. Owned by the
 * feature-infrastructure layer per the convention established by
 * `features/leads/infrastructure/lead.model.ts` and
 * `features/properties/infrastructure/property.model.ts` — the schema
 * file (`lib/db/schema/inquiry.ts`) describes the DB shape; the feature
 * layer derives its own `Row` / `Insert` aliases from it.
 *
 * These types are consumed by the mapper (I5) and the repository
 * adapter (I6). They are NOT the domain entity (`features/inquiries/
 * domain/inquiry.entity.ts`) — the domain version uses `undefined`
 * for nullable fields while the row uses `null`. The mapper bridges
 * the two boundaries.
 */
export type InquiryRow = typeof inquiry.$inferSelect
export type InquiryInsert = typeof inquiry.$inferInsert
