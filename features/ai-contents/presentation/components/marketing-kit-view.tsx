...
import { MarketingKitProgress } from "@/features/ai-contents/presentation/components/marketing-kit-progress"
import { MarketingKitCaptionGrid } from "@/features/ai-contents/presentation/components/marketing-kit-caption-grid"
import { MarketingKitContentCard } from "@/features/ai-contents/presentation/components/marketing-kit-content-card"
import { MarketingKitGeneratorDialog } from "@/features/ai-contents/presentation/components/marketing-kit-generator-dialog"
import { computeKitStatus } from "@/hooks/use-marketing-kit"
import { getAiContentsByPropertyAction, deleteAiContentsByPropertyAction } from "@/features/ai-contents/presentation/actions"
import type { Property } from "@/features/properties/domain/property.entity"
import type { AiContent, AiContentType, AiPlatform } from "@/features/ai-contents/domain/ai-content.entity"
...
export function MarketingKitView({ property }: MarketingKitViewProps) {
  ...
  const loadContents = useCallback(async () => {
    const data = await getAiContentsByPropertyAction(property.id)
    setContents(data)
    setLoading(false)
  }, [property.id])
  ...
  async function handleDeleteAll() {
    setDeleting(true)
    try {
      await deleteAiContentsByPropertyAction(property.id)
      await loadContents()
