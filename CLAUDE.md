...
- **Custom JWT claims** (injected by `custom_access_token_hook`, canonical source `drizzle/sql/012_custom_access_token_hook_nullif_trim.sql`, supersedes `003_*.sql`): `active_org_id`, `org_role`, `is_super_admin`, `user_name`. RLS policies and `rls.ts` read these. The `user_name` claim uses the same four-level fallback as `handle_new_user` in 011 (`full_name` → `name` → email local part → `'User'`) with `nullif(trim(...), '')` guards so whitespace-only metadata cannot leak into the UI.
