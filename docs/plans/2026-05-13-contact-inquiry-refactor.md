...
- [ ] **R38b** — `features/ai-contents` SECURITY: actions sin `getSessionContext()` + cross-org leak en mock store. ⚠️ **CRITICAL**. **Problema**: los 10 server actions en `features/ai-contents/presentation/actions.ts` llaman directo a infrastructure sin obtener session context. Cualquier agente autenticado puede invocar acciones cross-org. El in-memory store es process-global → datos de una org son visibles desde la sesión de otra org en el mismo server process. Pre-launch acceptable pero **NO debe llegar a producción**. **Solución**: wire `getSessionContext()` en cada action + cuando se migre a DB persistence, todas las queries via `withRLS(ctx, ...)` mandatorio (multitenancy zero-tolerance per CLAUDE.md). Dependiente de R38c (clean arch retrofit) y R38c-bis (DB persistence migration).
- [ ] **R38c** — `features/ai-contents` CLEAN ARCH: presentation importa infrastructure directo, sin application layer. **Problema**: `actions.ts` importa funciones de `infrastructure/ai-contents.service.ts` directamente. Viola la dependency rule de CLAUDE.md ("Presentation → Infrastructure: No, Never. Use cases mediate"). **Solución**: crear `features/ai-contents/application/` con use cases por operación (`get-contents.use-case.ts`, `create-content.use-case.ts`, `update-content.use-case.ts`, `delete-content.use-case.ts`, etc.). Refactor actions a thin pattern: auth + delegate to use case + return. Required para R38b auth wiring y para R38d DB persistence (use cases necesitan recibir repository via DI per Clean Arch).
- [ ] **R38d** — `features/ai-contents` DB PERSISTENCE: reemplazar in-memory mock stores con Drizzle repository. **Problema**: `ai-contents.service.ts` y `hashtags.store.ts` son in-memory arrays (`let contents: AiContent[]`). Cero persistencia. Datos se pierden en cada server restart. **Solución**: (1) Crear schema `lib/db/schema/ai-contents.ts` + `lib/db/schema/hashtags.ts` (o single table con `type` discriminator), (2) Domain repository interface `IAiContentRepository` en domain layer, (3) Drizzle adapter `DrizzleAiContentRepository implements IAiContentRepository` con todas las queries via `withRLS(ctx, ...)` + `organization_id NOT NULL` + RLS policies en migration nueva (SELECT/INSERT/UPDATE per org_id match), (4) Mapper Model ↔ Entity con null/undefined translation (proyecto convention), (5) Use cases en application layer (R38c) inyectan repo, (6) Actions wired con auth (R38b). Mirror del pattern Contact (R15-R17) / Inquiry (I4-I6) / Deal (R18-R20). Ticket grande — probablemente subtickets propios.
- [ ] **R38e** — `features/ai-contents` COPY: reemplazar mock data con seed neutral LATAM. **Problema**: mock data en `ai-contents.service.ts` contiene regionalismos argentinos ("pileta", "cochera", "Palermo, CABA", "Buenos Aires"). Producto apunta a LATAM wide + user en Bolivia. **Solución**: mock data debe usar localizaciones bolivianas/neutrales (Santa Cruz, La Paz, Cochabamba, terminología neutral). Cuando R38d migre a DB persistence, los mocks desaparecen y se reemplazan con seed scripts/fixtures correctas. Trivial pero importante para coherencia del producto.
...
- [ ] **R46c** — Standardise throw token casing across feature repos. Current state: `contacts/` repo (R17) uses `SCREAMING_SNAKE_CASE` (`CONTACT_NOT_FOUND`, ...); `inquiries/` repo (I6) uses `lowercase_snake_case` (`inquiry_not_found`, `deal_already_active`, ...) because the inquiry contract JSDoc enumerates them in lowercase. Decision required before Fase 6 server-action layer entrenches both styles in `try/catch` mapping. Reco: standardise to `lowercase_snake_case` (consistent with project's domain-error convention from acceptInvitation/bootstrapOrganization RPC tokens) — migrate Contact repo and update its docstring. Document the chosen convention in `CLAUDE.md` so future repos inherit.
