...
- [ ] **I8b** — `drizzle/sql/028_public_create_inquiry_rpc.sql` + `features/inquiries/presentation/public-actions.ts` + update `app/p/[id]/actions.ts` a llamar el nuevo path. SECURITY DEFINER RPC `public_create_inquiry(p_property_id, p_form_data)` que valida property (active + non-deleted), deriva `organization_id` + `created_by_user_id` server-side del owner, find-or-create Contact + INSERT Inquiry atómico, retorna inquiry id. EXECUTE granted to anon (mismo patrón `bootstrap_organization` / `accept_invitation`). Mantiene multitenancy zero-trust — anon nunca escoge la org. Replace legacy `createLeadAction` call en form público.
