...
**Estado:** ✅ **PASS COMPLETO 2026-04-25** post-fix G27 — verificación bidireccional via Playwright: switch Org H → Org A muestra T115 inmediatamente (sin reload manual); switch Org A → Org H oculta T115 + muestra empty list inmediatamente. Fix aplicado en commit del mismo día (revalidatePath layout server-side + window.location.reload client-side en OrgSwitcher). Originalmente PASS parcial 2026-04-25 por G27.
...
| G27 | **UX/SSR** | OrgSwitcher actualiza el header sidebar inmediatamente al cambiar de organización pero la lista de Properties NO se re-fetcha. Diagnóstico inicial pensaba que era server component con `revalidatePath` mal scoped — pero la causa real es que `/dashboard/properties/page.tsx` es **client component** (`"use client"`) que hace fetch via `useEffect` on mount. `router.refresh()` solo re-renderiza server components; no re-ejecuta useEffect en client components cuyas props no cambiaron | T124 (Lote 4 Bloque R) | Mini-fix en OrgSwitcher | ✅ **RESUELTO 2026-04-25.** Fix de 2 partes: (1) Server action `switchActiveOrgAction` cambia de `revalidatePath('/dashboard')` (page-only) a `revalidatePath('/dashboard', 'layout')` para invalidar todas las rutas anidadas. (2) Client OrgSwitcher reemplaza `router.refresh()` por `window.location.reload()` en `finally` block — full reload garantiza que cada page (server O client) refetcha con el nuevo `active_org_id` JWT, sin importar el patrón de fetching. `catch` con `console.error` para no swallow silently el error en startTransition (bubbles a Sentry/dev console). Code review (1 IMPORTANT + 1 MINOR) resueltos. Re-test T124 bidireccional ✅ — switch en cualquier dirección refresca lista automáticamente. Trade-off: brief flash en reload, aceptable porque org switch es operación rara (matches Slack/GitHub UX). |
