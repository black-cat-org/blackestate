...
| IMP-7 | **Auditoría RLS bypass exhaustiva** ✅ 2026-04-22 | Revisión completa de `db` directo en `features/shared/infrastructure/`. Resultado: 15 bypasses eliminados. Org repo y invitation repo enrutados por `withRLS` para toda query user-side, y RPC `SECURITY DEFINER` `accept_invitation` para el único caso que legítimamente cruza el boundary (aceptación antes de que el invitado sea miembro). Post-review fixes: restauración de member soft-deleted al aceptar invitación (restore vs `do nothing`), rollback tolerante a error en send, single-CTE snapshot en `getOrgSeatInfo`. Migraciones 007 + 008. Commits: 8bf3f2d, 6270c0c, 83d9797, + fixes. Nota 2026-04-24: RPC `bootstrap_organization` (también en 007) removida al adoptar el modelo 1-self-owned-org — ver `docs/plans/2026-04-24-remove-multi-org-creation.md`. | — |
